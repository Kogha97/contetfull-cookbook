...
export default function createEnvironmentApi(makeRequest) {
  const wrapEnvironment = entities.environment.wrapEnvironment;
  const _entities$contentType = entities.contentType,
  ...
  const _entities$asset = entities.asset,
    wrapAsset = _entities$asset.wrapAsset,
    wrapAssetCollection = _entities$asset.wrapAssetCollection;
  const wrapAssetKey = entities.assetKey.wrapAssetKey;
  const _entities$locale = entities.locale,
    wrapLocale = _entities$locale.wrapLocale,
    wrapLocaleCollection = _entities$locale.wrapLocaleCollection;
  const wrapSnapshotCollection = entities.snapshot.wrapSnapshotCollection;
  const _entities$editorInter = entities.editorInterface,
  ...
  const wrapUpload = entities.upload.wrapUpload;
  const _entities$extension = entities.extension,
    wrapExtension = _entities$extension.wrapExtension,
    wrapExtensionCollection = _entities$extension.wrapExtensionCollection;
  const _entities$appInstalla = entities.appInstallation,
  ...
  const wrapAppSignedRequest = entities.appSignedRequest.wrapAppSignedRequest;
  const wrapAppActionCall = entities.appActionCall.wrapAppActionCall;
  const wrapBulkAction = entities.bulkAction.wrapBulkAction;
  return {
    /**
    ...
    getUserUIConfig() {
      var _this2 = this;
      return _asyncToGenerator(function* () {
        const raw = _this2.toPlainObject();
        const data = yield makeRequest({
     ...
     * ```
     */
    getEnvironmentTemplateInstallations(environmentTemplateId, _ref = {}) {
      var _this3 = this;
      return _asyncToGenerator(function* () {
        let installationId = _ref.installationId,
          query = _objectWithoutProperties(_ref, _excluded);
        const raw = _this3.toPlainObject();
        return makeRequest({
